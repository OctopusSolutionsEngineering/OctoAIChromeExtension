...
**ALGORITHM — three valid feed sources (scan ALL of these, apply deduplication)**:
...
2. **Docker trigger `registry` field (primary fallback)**: used ONLY when `expectedArtifacts` has NO `docker/image` entries (absent, empty `[]`, or contains only non-docker types). This source is COMPLETELY DISABLED when any `docker/image` entry exists in `expectedArtifacts` — see the ABSOLUTE RULE above.
3. **Pubsub trigger `payloadConstraints.tag`**: extract the hostname from the tag value

**CRITICAL — the Docker trigger `registry` field is NEVER a supplemental feed source**: Even when the Docker trigger's registry differs from all `expectedArtifacts` docker/image registries (e.g., `expectedArtifacts` has `registry.example.invalid/image-X` and the Docker trigger has `registry: "gcr.io"`), the Docker trigger registry MUST NOT be used to create a second feed. The ABSOLUTE RULE above takes full precedence: the presence of ANY `docker/image` entry in `expectedArtifacts` disables the Docker trigger registry for feed creation entirely, without exception. This applies whether the Docker trigger is enabled (`"enabled": true`) or disabled (`"enabled": false`) — a disabled Docker trigger is still completely ignored for feed creation.

**MANDATORY SELF-CHECK before outputting any feed sections**: Before writing feed creation prompts, answer the following questions:
1. Does `expectedArtifacts` have ANY entry where `matchArtifact.type == "docker/image"`?
   - If YES → Source 1 applies. Source 2 (Docker trigger) is COMPLETELY DISABLED regardless of the Docker trigger's registry.
   - If NO → Source 2 (Docker trigger) applies as fallback (if a Docker trigger is present with a `registry` field).
2. Are there Pubsub triggers with `payloadConstraints.tag`? → Source 3 applies in all cases.
3. After collecting all URLs from applicable sources, apply deduplication: emit only ONE feed section per unique registry URL.

If you have produced a GCR feed section (`https://gcr.io/v2/`) AND the pipeline also has `expectedArtifacts` with `docker/image` entries, STOP — the GCR feed section from the Docker trigger must be removed. Only feed sections from `expectedArtifacts` docker/image entries and Pubsub trigger tags should remain.
...
**MANDATORY SELF-CHECK — project name character validity**: After deriving the project name, scan EVERY character in the final name and verify each one is a letter, digit, space, period, comma, dash, underscore, or hash. If any character fails this check, replace it with a dash `-` and collapse any resulting consecutive dashes before outputting the name.
...
**MANDATORY SELF-CHECK — step name validity scan**: Before finalizing the output, for EACH step you have generated, ensure the step name remains meaningfully derived from the original stage `name` while satisfying Octopus/Terraform naming constraints. Replace ALL characters that are not letters, digits, spaces, periods (`.`), commas (`,`), dashes (`-`), underscores (`_`), or hashes (`#`) with dashes `-`. This includes but is not limited to: parentheses `()`, square brackets `[]`, ampersands `&`, question marks `?`, exclamation points `!`, forward slashes `/`, colons `:`, plus signs `+`, equals signs `=`, asterisks `*`, at-signs `@`, percent signs `%`, vertical bars `|`. After replacing, collapse any sequence of two or more consecutive dashes into a single dash. Do not redact content with `*****`.
...
```
Create a project called "Deploy cronjob example to dev" in the "Default Project Group" project group with no steps.
* Add a community step template step with the name "Slack Notification - Start" and the URL "https://library.octopus.com/step-templates/99e6f203-3061-4018-9e34-4a3a9c3c3179" to the start of the deployment process. Set the "ssn_HookUrl" property to "#{Project.Slack.WebhookUrl}". Set the "ssn_Channel" property to "#pj-example-channel". Set the "ssn_Message" property to "#{Octopus.Project.Name} has started."
* Add a community step template step with the name "Slack Notification - Finish" and the URL "https://library.octopus.com/step-templates/99e6f203-3061-4018-9e34-4a3a9c3c3179" to the end of the deployment process. Only run the step when the previous step has failed. Set the "ssn_HookUrl" property to "#{Project.Slack.WebhookUrl}". Set the "ssn_Channel" property to "#pj-example-channel". Set the "ssn_Message" property to "#{Octopus.Project.Name} has failed."
* Add a community step template step with the name "Slack Notification - Complete" and the URL "https://library.octopus.com/step-templates/99e6f203-3061-4018-9e34-4a3a9c3c3179" to the end of the deployment process. Always run the step. Set the "ssn_HookUrl" property to "#{Project.Slack.WebhookUrl}". Set the "ssn_Channel" property to "#pj-example-channel". Set the "ssn_Message" property to "#{Octopus.Project.Name} has completed."
* Add a project variable called "enableAutomatedTrigger" with the value "false".
...
* **IMPORTANT**: The `message.pipeline.starting.text`, `message.pipeline.failed.text`, and `message.pipeline.complete.text` values must be copied verbatim from the Spinnaker pipeline JSON into the `ssn_Message` property. Do not modify, truncate, redact, summarize, or reword the text in any way. Copy the exact string character-for-character. In particular, do NOT add or remove trailing punctuation such as periods (`.`) — if the original text does not end with a period, the output must not end with a period either. **EXCEPTION — Spinnaker SpEL expressions must be converted**: When the message text contains Spinnaker SpEL expressions (`${...}` syntax) that appear in the mapping table below (e.g., `${execution.name}` → `#{Octopus.Project.Name}`, `${trigger['tag']}` → `#{Octopus.Deployment.Trigger.Name}`), replace them with the Octopus equivalents rather than copying verbatim. This SpEL conversion is the ONLY permitted deviation from verbatim copying. All other text (including service names, punctuation, and formatting) must remain exactly as written in the source JSON.
...
**Step 1 — Build the expected name list**: From the pipeline JSON `"stages"` array, extract the `"name"` field of every stage that is NOT a skipped type (`checkPreconditions`, `evaluateVariables`, `findArtifactFromExecution`, `findImage`, `findImageFromTags`, `tagImage`). **CRITICAL — custom Kubernetes Job stage types are NOT skipped**: stages of `"type": "shiftTrafficProd"`, `"type": "shiftTrafficStaging"`, `"type": "restoreProd"`, `"type": "deriveBaselineProd"`, and `"type": "deriveCanaryProd"` ARE converted to "Deploy Kubernetes YAML" steps and MUST appear in the expected name list. **CRITICAL — `pipeline` type stages are NOT skipped**: stages of `"type": "pipeline"` MUST appear in the expected name list because they each produce a "Deploy a Release" step. The step name for a `pipeline` stage is derived from the child project name extracted from the stage `"name"` field (e.g., stage `"name": "Run \"[DEV] Deploy API\""` → child project name `[DEV] Deploy API` → step name after replacing `[` and `]` with `-` → `"-DEV- Deploy API"`). Apply the step-name rules and write down every expected step name. This is your EXPECTED LIST.
**Step 1 — Build the expected name list**: From the pipeline JSON `"stages"` array, extract the `"name"` field of every stage that is NOT a skipped type (`checkPreconditions`, `evaluateVariables`, `findArtifactFromExecution`, `findImage`, `findImageFromTags`, `tagImage`). **CRITICAL — custom Kubernetes Job stage types are NOT skipped**: stages of `"type": "shiftTrafficProd"`, `"type": "shiftTrafficStaging"`, `"type": "restoreProd"`, `"type": "deriveBaselineProd"`, and `"type": "deriveCanaryProd"` ARE converted to "Deploy Kubernetes YAML" steps and MUST appear in the expected name list. **CRITICAL — `pipeline` type stages are NOT skipped**: stages of `"type": "pipeline"` MUST appear in the expected name list because they each produce a "Deploy a Release" step. The step name for a `pipeline` stage is derived from the child project name extracted from the stage `"name"` field (e.g., stage `"name": "Run \"[DEV] Deploy API\""` → child project name `[DEV] Deploy API` → step name after replacing `[` and `]` with `-` → `"-DEV- Deploy API"`). Apply the step-name rules and write down every expected step name. This is your EXPECTED LIST.
...
7. **IMPORTANT — Branch A continuation steps now implicitly wait for Branch B's root, regardless of whether Branch B has its own continuation stages**: When Branch B's root stage(s) are placed immediately after Branch A's root (as "Run in parallel with the previous step"), and Branch A's first continuation step uses "Wait for all previous steps to complete, then start", that continuation step will now wait for Branch B's root to complete as well — even if Branch B also has downstream continuation stages. In the original Spinnaker pipeline, the Branch A continuation stage depended ONLY on Branch A's root — it did NOT wait for Branch B's root. This is ALWAYS a behavior change: **add the following migration NOTE to Branch A's FIRST continuation step in every fork-without-reconvergence pipeline, whether or not Branch B is a leaf**: `NOTE (migration): In the original Spinnaker pipeline, this step depended only on "<Branch A root stage name>" and did not wait for "<Branch B root stage(s)>". In this Octopus migration, this step implicitly waits for all preceding parallel steps including the Branch B stages.` Replace `<Branch A root stage name>` and `<Branch B root stage(s)>` with the actual stage names.

**Worked example — 2-branch fork where BOTH branches have continuation stages (e.g., a pipeline with 5 stages: 1→[2→4, 3→5])**:

Given a pipeline: Stage 1 (Deploy ConfigMap) fans out to Stage 2 (Manual Judgment (Deploy All)) and Stage 3 (Manual Judgment (Migrate)). Stage 4 (Deploy All) depends only on Stage 2. Stage 5 (Execute DB Migration) depends only on Stage 3. This is a fork-without-reconvergence where BOTH branches have continuations.

The **CORRECT** output (with NOTEs for both branches):
```
* Add a "Deploy Kubernetes YAML" step with the name "Deploy ConfigMap" ...   ← stage 1, sequential root

* Add a "Manual Intervention" step with the name "Manual Judgment -Deploy All-" ... Set the start trigger to "Wait for all previous steps to complete, then start".   ← stage 2, Branch A root (first in {2,3} parallel group)

* Add a "Manual Intervention" step with the name "Manual Judgment -Migrate-" ... Set the start trigger to "Run in parallel with the previous step".   ← stage 3, Branch B root (second in {2,3} parallel group)

* Add a "Deploy Kubernetes YAML" step with the name "Deploy All" ... Set the start trigger to "Wait for all previous steps to complete, then start". Set the step description to "...; NOTE (migration): In the original Spinnaker pipeline, this step depended only on 'Manual Judgment (Deploy All)' and did not wait for 'Manual Judgment (Migrate)'. In this Octopus migration, this step implicitly waits for all preceding parallel steps including the Branch B stages."   ← stage 4, Branch A continuation (NOTE required — implicitly waits for Stage 3)

* Add a step with the name "Execute DB Migration" ... Set the start trigger to "Wait for all previous steps to complete, then start". Set the step description to "...; NOTE (migration): In the original Spinnaker pipeline, this step ran concurrently with the 'Manual Judgment (Deploy All)' / 'Deploy All' path. In this Octopus migration, these steps have been linearized and will run sequentially."   ← stage 5, Branch B continuation (NOTE required — now sequentialized after entire Branch A)
```

← CORRECT: Both NOTEs are present. Deploy All notes that it now waits for Manual Judgment (Migrate). Execute DB Migration notes that it now runs after the entire Deploy All chain (not just after Manual Judgment (Migrate)).

The **WRONG** output omits the branch NOTEs (FORBIDDEN):
```
* Add a "Manual Intervention" step ... "Manual Judgment -Deploy All-" ... Set the start trigger to "Wait for all previous steps to complete, then start".
* Add a "Manual Intervention" step ... "Manual Judgment -Migrate-" ... Set the start trigger to "Run in parallel with the previous step".
* Add a "Deploy Kubernetes YAML" step ... "Deploy All" ... Set the start trigger to "Wait for all previous steps to complete, then start".   ← WRONG: missing NOTE about waiting for Manual Judgment (Migrate)
* Add a step ... "Execute DB Migration" ... Set the start trigger to "Wait for all previous steps to complete, then start".   ← WRONG: missing NOTE about running after entire Deploy All path
```
← WRONG: Engineers cannot tell from the migration output that these steps no longer run concurrently and that Deploy All now waits for Manual Judgment (Migrate) to complete.
...
**ABSOLUTE RULE — the ` 2` suffix applies ONLY when two stages have IDENTICAL names after the step-name rules are applied**: The deduplication suffix (` 2`, ` 3`, etc.) is appended ONLY when two or more stages produce the same step name after applying the name transformation rules. If stage A is named "Manual Judgment (Deploy Canary)" and stage B is named "Manual Judgement (Cronjob)", these are DIFFERENT names — stage B must be named `"Manual Judgement (Cronjob)"`, NOT `"Manual Judgment (Deploy Canary) 2"`. The fact that stages A and B are in the same parallel group does NOT cause them to share a name. Check the actual stage `name` field from the JSON before applying any deduplication suffix.
