...
const platformPrompts = {
    ...
    kubernetesmicroservices: '* Create a token account called "Mock Token".\n* Create a feed called "Docker Hub" pointing to "https://index.docker.io" using anonymous authentication.\n* Add a target called "Mock K8s", with the tag "Kubernetes", using the token account, pointing to "https://mockk8s.octopusdemos.com", using the health check image "octopusdeploy/worker-tools:6.5.0-ubuntu.22.04" from the "Docker Hub" feed, using the worker pool "Hosted Ubuntu".\n\n---\n\nCreate a Kubernetes project called "K8s Microservice 1", and then:\n* Place the project in the "Orchestrator" project group.\n* Configure the Kubernetes steps to use client side apply (client side apply is required by the "Mock K8s" target).\n* Disable verification checks in the Kubernetes steps (verification checks are not supported by the "Mock K8s" target).\n* Enable retries on the Kubernetes step.\n\n---\n\nCreate a Kubernetes project called "K8s Microservice 2", and then:\n* Place the project in the "Orchestrator" project group.\n* Configure the Kubernetes steps to use client side apply (client side apply is required by the "Mock K8s" target).\n* Disable verification checks in the Kubernetes steps (verification checks are not supported by the "Mock K8s" target).\n* Enable retries on the Kubernetes step.\n\n---\n\nCreate an Orchestration project called "Kubernetes Microservice Orchestration" managing the projects "K8s Microservice 1" and "K8s Microservice 2".',
    argocd: 'Create an Argo CD Tag Update project called "My Argo CD Tag Update" with the slug "argo-cd-octopub", and create a Git Connection called "Mock" with a random username add the allowed repository "https://mockgit.octopusdemos.com/*" in the repository restrictions',
    argocdmanifest: 'Create an Argo CD Manifest Update project called "My Argo CD Manifest Update" with the slug "argo-cd-octopub-manifest", and create a Git Connection called "Mock" with a random username add the allowed repository "https://mockgit.octopusdemos.com/*" in the repository restrictions',
    ...
    platformhub: 'Configure the Platform Hub git repo to point to https://mockgit.octopusdemos.com/repo/platformhubrepo using the ".octopus" base path.'
};
